...
export default function Receipt({ selectedData, personData, formWalkinData }) {
  ...
  const handleSave = async (e) => {
    try {
      ...
      if (personData) {
        await packageService.insertCusPackage(
          selectedData,
          ...
          <tbody>
            {selectedData.map((item, index) => (
              <tr className='hover:bg-gray-50' key={index}>
                ...
                <td className='border border-gray-300 px-3 py-2 text-center'>
                  {discountShow ? `$${item.price}` : `$${item.subPrice}`}
                </td>
                {discountShow && (
                  <td className='border border-gray-300 px-3 py-2 text-center'>
          ...
          <div className='flex gap-9'>
            <label className='inline-flex items-center bg-gray-300 p-3 rounded-sm w-40'>
              <input
                ...
                name='paymentMethod'
                value='card'
                onChange={!saved && handleChange}
                checked={formPaymentData.paymentMethod == "card"}
                className='form-radio text-blue-600'
              />
            ...
            <label className='inline-flex items-center bg-gray-300 p-3 rounded-sm w-40'>
              <input
                ...
                value='cash'
                onChange={!saved && handleChange}
                checked={formPaymentData.paymentMethod == "cash"}
                className='form-radio text-blue-600'
              />
            ...
            <label className='inline-flex items-center bg-gray-300 p-3 rounded-sm w-40'>
              <input
                ...
                value='paynow'
                onChange={!saved && handleChange}
                checked={formPaymentData.paymentMethod == "paynow"}
                className='form-radio text-blue-600'
              />
