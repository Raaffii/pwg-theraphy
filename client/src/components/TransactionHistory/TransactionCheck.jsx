...
import PrintReceipt from "../InvoiceReceipt/PrintReceipt";
import { useAuth } from "@/context/AuthContext";
import { Input } from "@/components/ui/input";
import { ReceiptDocument } from "../InvoiceReceipt/ReceiptDocument";
...
export default function TrasactionCheck({ idPosHd, customerData, dataPosHd }) {
  ...
  const totalPrice = dataPosline
    .reduce((total, item) => total + item.total_price * 1, 0)
    .toFixed(2);
        ...
        <tbody>
          {dataPosline.map((item, index) => (
            <tr className='hover:bg-gray-50' key={index}>
              ...
              <td className='border border-gray-300 px-3 py-2 text-center'>
                {item.discpercent ? `${item.discount}%` : `-$${item.discount}`}
              </td>
              <td className='border border-gray-300 px-3 py-2 text-center'>
                ${item.total_price}
              </td>
            ...
            {!(formPaymentData.receiptOption == "printReceipt") && (
              <label className='inline-flex items-center'>
                <input
                  ...
                  checked={formPaymentData.receiptOption == "emailReceipt"}
            ...
            {!(formPaymentData.receiptOption == "emailReceipt") && (
              <label className='inline-flex items-center'>
                <input
                  ...
                  value='printReceipt'
                  onChange={handleChange}
                  checked={formPaymentData.receiptOption == "printReceipt"}
                  className='form-radio text-blue-600'
                  onClick={handleClick}
                />
